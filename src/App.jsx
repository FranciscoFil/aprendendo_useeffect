...
function App() {
  ...
  useEffect(() => {
    fetch("https://api.github.com/users/FranciscoFil")
    .then((res) => res.json())
  ...
  return ( <div className='App'>
        <div>
          <button onClick={() => setCount(prevCount => prevCount + 1)}>Rederizar</button>
          <p>{count}</p>
        </div>
        <div>
          <button onClick={() => setCountB(prevCount => prevCount + 1)}>Rederizar B</button>
          <p>{countB}</p>
        </div>
        {user && (
          <div>
